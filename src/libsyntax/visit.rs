...
pub fn walk_inlined_item<E: Clone, V: Visitor<E>>(visitor: &mut V,
                                                  ...
                                                  env: E) {
    match *item {
        IIItem(ref i) => visitor.visit_item(&**i, env),
        IIForeign(ref i) => visitor.visit_foreign_item(&**i, env),
        IIMethod(_, _, ref m) => walk_method_helper(visitor, &**m, env),
    }
}
...
pub fn walk_local<E: Clone, V: Visitor<E>>(visitor: &mut V, local: &Local, env: E) {
    ...
    match local.init {
        None => {}
        Some(ref initializer) => visitor.visit_expr(&**initializer, env),
    }
}
...
pub fn walk_ty<E: Clone, V: Visitor<E>>(visitor: &mut V, typ: &Ty, env: E) {
    match typ.node {
        TyUniq(ref ty) | TyVec(ref ty) | TyBox(ref ty) => {
...
pub fn walk_foreign_item<E: Clone, V: Visitor<E>>(visitor: &mut V,
    ...
    match foreign_item.node {
        ...
        ForeignItemStatic(ref typ, _) => visitor.visit_ty(&**typ, env),
    }
}
...
pub fn walk_expr_opt<E: Clone, V: Visitor<E>>(visitor: &mut V,
                                              optional_expression: &Option<P<Expr>>,
                                              env: E) {
