...
use abi;
use ast::{StaticRegionTyParamBound, OtherRegionTyParamBound,
...
impl<'a> State<'a> {
    ...
    pub fn print_type(&mut self, ty: &ast::Ty) -> IoResult<()> {
        ...
        match ty.node {
            ...
            ast::TyBareFn(ref f) => {
                ...
                try!(self.print_ty_fn(Some(f.abi), None, &None, f.fn_style,
                                      ast::Many, &*f.decl, None, &None,
                                      Some(&generics), None));
            }
    ...
    pub fn print_foreign_item(&mut self,
        ...
        match item.node {
            ast::ForeignItemFn(ref decl, ref generics) => {
                try!(self.print_fn(&**decl, None, abi::Rust, item.ident,
    ...
    pub fn print_expr(&mut self, expr: &ast::Expr) -> IoResult<()> {
        ...
        match expr.node {
            ...
            ast::ExprInlineAsm(ref a) => {
                ...
                try!(self.commasep(Inconsistent, a.outputs.as_slice(), |s, &(ref co, ref o)| {
                ...
                try!(self.commasep(Inconsistent, a.inputs.as_slice(), |s, &(ref co, ref o)| {
                    try!(s.print_string(co.get(), ast::CookedStr));
                    try!(s.popen());
                    try!(s.print_expr(&**o));
                    try!(s.pclose());
    ...
    pub fn print_pat(&mut self, pat: &ast::Pat) -> IoResult<()> {
        ...
        match pat.node {
            ...
            ast::PatIdent(binding_mode, ref path, ref sub) => {
                match binding_mode {
                    ast::BindByRef(mutbl) => {
