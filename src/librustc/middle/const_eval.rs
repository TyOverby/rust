...
fn lookup_variant_by_id<'a>(tcx: &'a ty::ctxt,
    ...
    if ast_util::is_local(enum_def) {
        ...
        let e = match csearch::maybe_get_item_ast(tcx, enum_def,
            ...
            csearch::found(ast::IIItemRef(item)) => match item.node {
                ItemEnum(ast::EnumDef { variants: ref variants }, _) => {
                    variant_expr(variants.as_slice(), variant_def.node)
                }
                _ => None
        ...
        tcx.extern_const_variants.borrow_mut().insert(variant_def, e.map(|e| e as *_));
        e
    }
}
...
pub fn lookup_const_by_id<'a>(tcx: &'a ty::ctxt, def_id: ast::DefId)
    ...
    if ast_util::is_local(def_id) {
        ...
        let e = match csearch::maybe_get_item_ast(tcx, def_id,
            |a, b, c, d| astencode::decode_inlined_item(a, b, c, d)) {
            csearch::found(ast::IIItemRef(item)) => match item.node {
                ItemStatic(_, ast::MutImmutable, ref const_expr) => Some(&**const_expr),
                _ => None
            },
            _ => None
        };
        tcx.extern_const_statics.borrow_mut().insert(def_id, e.map(|e| e as *_));
        e
    }
}
...
impl<'a> ConstEvalVisitor<'a> {
    fn classify(&mut self, e: &Expr) -> constness {
        ...
        let cn = match e.node {
            ...
            ast::ExprAddrOf(ast::MutImmutable, ref base) => self.classify(&**base),
